...
test('E2E: mark backlog item as watched via UI', async ({ page, request }) => {
  ...
  await clickActionForTitle(page, expectedTitle, 'Mark as Watched');
  await waitForPatch;

  // confirm DB updated
  const after = await request.get('http://127.0.0.1:3000/backlog/sorted', {
    headers: { Authorization: `Bearer ${tokens.accessToken}` },
  });
  const afterRows = await after.json();
  const updated = afterRows.find((r) => r.movie_show_id === movie.id);
  expect(updated, 'backlog entry not found').toBeTruthy();
