...
### Sequence Diagram
<img width="846" height="554" alt="diagram" src="https://github.com/user-attachments/assets/bcdecda1-3842-4497-8d6a-c96a637597d9" />

This sequence diagram focuses on the movie search process and what the user receives on the search results page. This diagram and specific interaction demonstrate a key part of our app, as it showcases the interaction between three separate parts: the TMDB API, MySQL's streaming_service database (our two data components, and the frontend's search input. As a result, the diagram explains how movie information and streaming availability are retrieved and displayed on the frontend UI. By specifically focusing on the appearance of the "STREAM" badge, the sequence diagram shows the logic of how our app finds movies that match the user's streaming services. By retrieving the selectedServices from the selected_services database and TMDB's providers, it enters a loop for every movie result, creating a PosterCard to display on the screen. For each movie, it compares the user's selectedServices with TMDB's providers to determine whether there is at least one common service/provider. If so, it adds a "STREAM" badge to the PosterCard. This comparison between the two data sources demonstrates a core feature of our project, since the functionality of determining whether new media or saved media is available to stream is seen across other pages.
